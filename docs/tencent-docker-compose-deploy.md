...
- `TENCENT_DEPLOY_PATH`：部署目录，默认 `$HOME/medipolicyai`
...
首次运行 workflow 时，如果服务器没有 `$HOME/medipolicyai/shared/.env`，会自动生成模板并主动失败。登录服务器补齐 `LLM_API_KEY` 后，重新运行 workflow。

```bash
ssh root@你的服务器
vim "$HOME/medipolicyai/shared/.env"
```
